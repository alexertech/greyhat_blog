...
RSpec.describe DashboardsController, type: :controller do
  ...
  describe 'private methods' do
    ...
    describe '#extract_popular_search_terms' do
      before do
        allow(controller_instance).to receive(:authenticate_user!)
        create(:visit, referer: 'https://google.com/search?q=ruby+programming')
        create(:visit, referer: 'https://google.com/search?q=rails+tutorial')
        create(:visit, referer: 'https://bing.com/search?q=ruby+programming')
      end

      it 'extracts search terms from referrers' do
        terms = controller_instance.send(:extract_popular_search_terms)
        expect(terms).to be_a(Hash)
      end

      it 'handles invalid URLs gracefully' do
        create(:visit, referer: 'invalid-url')
        expect { controller_instance.send(:extract_popular_search_terms) }.not_to raise_error
      end
    end

    describe '#analyze_user_agents' do
      before do
        allow(controller_instance).to receive(:authenticate_user!)
        create(:visit, user_agent: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/91.0')
        create(:visit, user_agent: 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7) Safari/14.1')
        create(:visit, user_agent: 'Mozilla/5.0 (X11; Linux x86_64) Firefox/89.0')
        create(:visit, user_agent: 'Mozilla/5.0 (iPhone; CPU iPhone OS 14_6 like Mac OS X) Mobile')
      end

      it 'categorizes browsers correctly' do
        analysis = controller_instance.send(:analyze_user_agents)
        expect(analysis[:browsers]).to have_key('Chrome')
        expect(analysis[:browsers]).to have_key('Safari')
        expect(analysis[:browsers]).to have_key('Firefox')
      end

      it 'categorizes devices correctly' do
        analysis = controller_instance.send(:analyze_user_agents)
        expect(analysis[:devices]).to have_key('Desktop')
        expect(analysis[:devices]).to have_key('Mobile')
      end

      it 'categorizes operating systems correctly' do
        analysis = controller_instance.send(:analyze_user_agents)
        expect(analysis[:operating_systems]).to have_key('Windows')
        expect(analysis[:operating_systems]).to have_key('macOS')
        expect(analysis[:operating_systems]).to have_key('Linux')
        expect(analysis[:operating_systems]).to have_key('iOS')
      end
    end
  end
end